...
* **🔗 Symbolic Links Are Not Followed:** Symbolic links (including Windows junctions) are excluded from content extraction. A link pointing outside the analyzed folder never leaks its target into the output, and a link that loops never makes the traversal hang. The link itself is still listed in the folder tree, annotated with `[symbolic link, not followed]`.
